...
class App extends Component {
	...
	deleteGoal = (isDaily, categoryLoc, goalLoc) => {
		if(isDaily){
			this.setState(update(this.state, {goals: {dailyGoals: {$splice: [[goalLoc, 1]]}} } ))
		}
		else{
			this.setState(update(this.state, {goals: {otherGoalsCategories: {[categoryLoc]: {otherGoals: {$splice: [[goalLoc, 1]]}} } } } ))
		}
	}
	completeGoal = (categoryLoc, goalLoc) => {
		// let otherGoals = this.state.goals.otherGoalsCategories[categoryLoc].otherGoals;
		// let updatedState;
		// if(goalLoc === 0){
		// 	this.setState(update(this.state, {goals: {otherGoalsCategories: {[categoryLoc]: {otherGoals: {[goalLoc]: {isCompleted:{$set: true}}}} } } } ))
		// 	return;
		// }
		// if(!otherGoals[0].isCompleted){
		// 	updatedState = update(this.state, {goals: {otherGoalsCategories: {[categoryLoc]: {otherGoals: {[goalLoc]: {isCompleted:{$set: true}}}} } } } )
		// 	updatedState = update(this.state, {goals: {otherGoalsCategories: {[categoryLoc]: {otherGoals: {$shift: [otherGoals[goalLoc]]}} } } } )
		// 	console.log(updatedState)
		// 	updatedState = update(this.state, {goals: {otherGoalsCategories: {[categoryLoc]: {otherGoals: {$splice: [[goalLoc+, 1]]}} } } } )
				...
				<div className="main">
					...
					<div className="goals">
						...
						{state.otherStuffs.renderOther && state.otherStuffs.renderCurrent && state.goals.otherGoalsCategories.length !== 0
							&& <OtherGoals otherGoalCategories={state.goals.otherGoalsCategories} deleteGoal={this.deleteGoal} completeGoal={this.completeGoal} displayCompleted={state.otherStuffs.renderCompleted} />}
